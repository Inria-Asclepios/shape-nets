...
class shape_dataset(Dataset):
    def __init__(self, data, labels, representation_names=None, k_eig=128, normalise_rep=False,
        ...
        if self.use_cache:
            ...
            for i, shape in enumerate(self.data):
                ...
                if self.representation_names is None:
                    reps = np.zeros(shape.n_points)
                    self.normalise_rep = False
                ...
                for r in self.representation_names:
                    ...
                    if r == 'shot':
                        shot_reps = shape.point_data[r]
                        shot_reps = torch.tensor(np.ascontiguousarray(shot_reps)).float()
                ...
                self.all_reps.append([k_reps, k1k2_reps, hks_reps, shot_reps])
