...
def mesh_sampler(vertices, faces, features, normals, n_points=1024):
    ...
    final_normals = normals[sampled_idxs]

    return final_verts, final_features, final_normals


class ShapeDataset(Dataset):
    def __init__(self, data_list, labels, input_name=None, n_points=1000, pre_transform=None):
        self.labels = labels
        ...
        for i, shape in tqdm(enumerate(self.data_list)):
            ...
            if self.pre_transform is not None:
                data = Data(pos=verts, face=faces)
            ...
            if self.n_points is not None:
                verts, reps, _ = mesh_sampler(verts, faces, reps, normals, self.n_points)
