    ...
    for i in range(n_fold):
        ...
        for epoch in tqdm(range(n_epoch)):
            ...
            for data in train_loader:
                ...
                preds = model(data)
                labels = data.y.long()
                loss = diffusion_net.utils.label_smoothing_log_loss(preds[0], labels, label_smoothing)
...
def main(base_fold):
    logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(name)s - %(levelname)s - %(message)s',
                        datefmt='%d/%m/%Y %H:%M:%S')
    device = 'cuda'
    datasets = ['AtriumAF', 'Shrec_16_coarse']
    for dataset in datasets:
        ...
        if not exists(res_fold):
            os.mkdir(res_fold)
        split_name = 'split_'
        if dataset == 'Shrec_16_coarse':
            n = 30
        else:
            n = 2

        for f in ['shot16', 'k1k2n', 'k', 'k1k2', 'xyz', 'hks', 'shot']:
            res = join(res_fold, f)
            run_classification_model(data_dir=data, info_df=info, res_dir=res, label_name='labels',
                                     n_fold=1, split_name_base=split_name,
                                     n_class=n, input_features=f, n_points=None,
                                     n_epoch=2, lr=1e-3, scheduler_step=50, scheduler_gamma=.5,
                                     device_name=device)
