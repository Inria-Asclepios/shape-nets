...
def run_segmentation_model(data_dir, info_df, res_dir, label_name='labels', cached_data=None,
                           n_fold=5, split_name_base='split_', random_rotate=False,
    ...
    if device_name == 'mps':
        ...
        logging.info("Metal doesn't support Sparse objects yet, will have to change them to dense")
    ...
    if input_features == 'k':
        ...
        c_in = 2
    elif input_features in ['xyz', 'nor']:
        c_in = 3
    ...
    n_fold_test_ids = [np.argwhere(info_df[splits[i]] == 1).flatten() for i in range(n_fold)]
    ...
    for i in range(n_fold):
        ...
        for epoch in tqdm(range(n_epoch)):
            ...
            for data in train_loader:
                ...
                tracker.update(pred_labels.to(device), labels.to(device))
            scheduler.step()
            ...
            with torch.no_grad():
                for data in test_loader:
                    ...
                    preds = model(feats, mass, L=L, evals=evals, evecs=evecs, gradX=gradX, gradY=gradY, faces=faces)
                    pred_labels = torch.argmax(preds, dim=-1)
                    tracker_test.update(pred_labels.to(device), labels.to(device))
