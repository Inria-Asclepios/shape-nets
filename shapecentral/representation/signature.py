...
class SignatureExtractor(object):
    ...
    def initialize(self,
        ...
        self.evals, self.evecs = scipy.sparse.linalg.eigsh(self.W, self.n_basis, self.M, sigma=sigma, OPinv=op_inv)
    ...
    def heat_signatures(self,
        ...
        if times is None:
            ...
            assert len(
                times) == dim, f"Requested feature dimension and time steps array do not match: {dim} and {len(times)}"
    ...
    def wave_distance(self,
        ...
        Args:
            query (int, arrylike): queried indices
            dim (int): target index
            return_signatures (bool): If True the function returns a tuple (distances, signatures). Defaults to False
            energies (None, arraylike): Time steps used for signature computation. Defaults to None
            cutoff (float, optional): Fraction of dimensionality to use for distance computation. Defaults to 1.0
    ...
    def feature_distance(self,
        ...
        Args:
            query (int, arraylike): queried indices
            dim (int): target index
