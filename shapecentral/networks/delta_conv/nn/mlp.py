import torch
from torch.nn import Sequential as Seq, Linear as Lin, BatchNorm1d, LeakyReLU, InstanceNorm1d

from .nonlin import BatchNorm1d, VectorNonLin


def MLP(channels, bias=False, nonlin=LeakyReLU(negative_slope=0.2), bn=True):
    if bn:
        seq = Seq(*[Seq(Lin(channels[i - 1], channels[i], bias=bias), BatchNorm1d(channels[i]), nonlin)
                    for i in range(1, len(channels))])
    else:
        seq = Seq(*[Seq(Lin(channels[i - 1], channels[i], bias=bias), nonlin) for i in range(1, len(channels))])
    return seq


def VectorMLP(channels, batchnorm=True):
    return Seq(*[
        Seq(Lin(channels[i - 1], channels[i], bias=False),
