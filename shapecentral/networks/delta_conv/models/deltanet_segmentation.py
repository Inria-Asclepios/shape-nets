...
class DeltaNetSegmentation(torch.nn.Module):
    def __init__(self, in_channels, num_classes, conv_channels=None, mlp_depth=2, embedding_size=1024,
                 categorical_vector=False, num_neighbors=20, grad_regularizer=0.001, grad_kernel_width=1):
        ...
        if categorical_vector:
            ...
            self.segmentation_head = Seq(
                MLP([embedding_size + sum(conv_channels), 256]),
                Dropout(0.5),
                MLP([256, 256]),
                Dropout(0.5),
                Linear(256, 128),
